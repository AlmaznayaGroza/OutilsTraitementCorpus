...
augmented_stats = (
    augmented_df.groupby(["language", "source"])
    .agg(
...
lang_stats = (
    combined_df.groupby(["language", "source_corpus"])
    .agg(avg_tokens=("token_count", "mean"))
    .reset_index()
)

lang_pivot = lang_stats.pivot(
    index="language", columns="source_corpus", values="avg_tokens"
).reset_index()
lang_pivot["diff_pct"] = (
    (lang_pivot["augmented"] - lang_pivot["original"]) / lang_pivot["original"] * 100
)
...
pivot = pd.crosstab(
    index=augmented_df["language"], columns=augmented_df["source"], margins=False
)
...
for source_type in [
    ...
    samples = augmented_df[augmented_df["source"] == source_type].sample(
        min(3, len(augmented_df[augmented_df["source"] == source_type]))
    )
