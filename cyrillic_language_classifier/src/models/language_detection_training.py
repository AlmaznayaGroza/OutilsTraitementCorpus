# -*- coding: utf-8 -*-
"""language_detection_training.ipynb
...
def visualize_common_errors(error_analysis, title="Les 10 confusions les lus fréquentes"):
    """
...
# ==============================================
# SAUVEGARDE ET TÉLÉCHARGEMENT DES RÉSULTATS
# Cette cellule sécurise tous tes résultats
# avant de continuer sur les dernières analyses
# ==============================================

logger.info("SAUVEGARDE DE TOUS LES RÉSULTATS")

from google.colab import files

# --- ÉTAPE 1: INVENTAIRE DES RÉSULTATS ---
def inventory_results():
    """Fait l'inventaire de tous les fichiers de résultats créés."""
    logger.info("📋 Inventaire des résultats créés...")
        ...
        if os.path.exists(directory):
            for root, dirs, files in os.walk(directory):
                for file in files:
                    ...
                    results_inventory[category].append({
                        'name': file,
                        'path': file_path,
    ...
    logger.info(f"📊 Total: {total_files} fichiers, {total_size / (1024 * 1024):.1f} MB")

    # Afficher l'inventaire détaillé
    for category, files_list in results_inventory.items():
        if files_list:
            logger.info(f"  • {category.capitalize()}: {len(files_list)} fichiers")
            for file_info in files_list:
                logger.info(f"    - {file_info['name']} ({file_info['size_mb']:.1f} MB)")

    return results_inventory, total_files, total_size

# Faire l'inventaire
inventory, file_count, total_size = inventory_results()

# --- ÉTAPE 2: CRÉATION DE L'ARCHIVE COMPLÈTE ---
def create_comprehensive_archive():
    """Crée une archive ZIP complète de tous les résultats."""
    logger.info("Création de l'archive complète...")

    archive_path = "/content/cyrillic_language_detection_results.zip"

    with zipfile.ZipFile(archive_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # Ajouter tous les répertoires de résultats
        base_dirs = [
            '/content/results',
            '/content/logs'
        ]
        ...
        for base_dir in base_dirs:
            if os.path.exists(base_dir):
                for root, dirs, files in os.walk(base_dir):
                    for file in files:
                        file_path = os.path.join(root, file)
                        # Créer un nom d'archive relatif propre
                        archive_name = os.path.relpath(file_path, '/content')
                        zipf.write(file_path, archive_name)
                        files_added += 1

        # Ajouter un résumé du projet dans l'archive
        project_summary = f"""
PROJET: Détection Automatique de Langues Cyrilliques
========================================================

RÉSULTATS PRINCIPAUX:
• Modèle entraîné sur {len(all_languages)} langues cyrilliques
• Précision finale: {performance_report['global_metrics']['accuracy']:.4f}
• Calibration ECE: {calibration_analysis['expected_calibration_error']:.4f}
• Total erreurs: {error_analysis['total_errors']} sur {prediction_results['num_examples']} exemples

CONTENU DE L'ARCHIVE:
• Modèle final entraîné (language-detection-final/)
• {len(inventory['figures'])} visualisations (results/figures/)
• {len(inventory['metrics'])} fichiers de métriques (results/metrics/)
• {len(inventory['logs'])} fichiers de logs (logs/)

STATISTIQUES TECHNIQUES:
• Total fichiers: {file_count}
...
✅ Analyse des erreurs par pattern
✅ Performance par langue individuelle
✅ Analyse par groupes linguistiques
✅ Calibration et métacognition
✅ Robustesse à la longueur des textes
✅ Corrélations données-performance

Ce projet implémente:
- préparation et augmentation de corpus linguistiques
- fine-tuning de modèles Transformers
- évaluation approfondie et méthodique
- visualisation scientifique de résultats
- analyse critique des performances
        """.strip()

        # Ajouter le résumé à l'archive
        zipf.writestr("PROJECT_SUMMARY.txt", project_summary.encode('utf-8'))
        files_added += 1

    archive_size = os.path.getsize(archive_path)
    logger.info(f"✅ Archive créée: {files_added} fichiers, {archive_size / (1024 * 1024):.1f} MB")

    return archive_path, archive_size

# Créer l'archive
archive_path, archive_size = create_comprehensive_archive()

# --- ÉTAPE 3: TÉLÉCHARGEMENT AUTOMATIQUE ---
def download_results():
    """Télécharge automatiquement l'archive de résultats."""
    logger.info("💾 Téléchargement de l'archive...")

    try:
        # Télécharger l'archive principale
        files.download(archive_path)
        logger.info("✅ Archive téléchargée avec succès!")

        # Optionnel: télécharger aussi des fichiers critiques individuellement
        critical_files = [
        ...
        logger.info("📎 Téléchargement de fichiers critiques supplémentaires...")
        for file_path in critical_files:
            if os.path.exists(file_path):
                try:
                    files.download(file_path)
                except:
                    logger.warning(f"Impossible de télécharger {file_path}")

        return True

    except Exception as e:
        logger.error(f"❌ Erreur lors du téléchargement: {e}")
        logger.info("💡 Alternative: l'archive est sauvegardée dans /content/cyrillic_language_detection_results.zip")
        return False

# Lancer le téléchargement
download_success = download_results()

# --- ÉTAPE 4: INSTRUCTIONS DE RÉCUPÉRATION ALTERNATIVES ---
logger.info("="*80)
logger.info("🔒 INSTRUCTIONS DE SAUVEGARDE")
logger.info("="*80)

if download_success:
    logger.info("✅ Téléchargement automatique réussi!")
    logger.info("Les résultats sont maintenant en sécurité en local.")
else:
    logger.info("⚠️ Si le téléchargement automatique échoue, voici les alternatives:")
    logger.info("1. Archive principale: /content/cyrillic_language_detection_results.zip")
    logger.info("2. Modèle entraîné: /content/results/models/language-detection-final/")
    logger.info("3. Visualisations: /content/results/figures/")
    logger.info("4. Métriques: /content/results/metrics/")

logger.info("="*80)
logger.info("PRÊT POUR LES ANALYSES COMPLÉMENTAIRES")
logger.info("="*80)
logger.info("Les résultats sont en sécurité!")
logger.info("="*80)
...
def visualize_augmentation_performance(augmentation_analysis):
    ...
    # Deuxième graphique: Confiance par type d'augmentation
