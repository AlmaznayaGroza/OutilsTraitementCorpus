# Script de collecte ciblée pour les langues où il y a eu des erreurs
# (reprend le script de base)
import requests
import pandas as pd
import time
import random
...
# Limiter le temps à 6 heure par langue environ (script déjà très long)
TIME_LIMIT = 22000 

# Définition des groupes de langues basés sur les résultats d'un premier test
LANGUAGE_GROUPS = {
    'A': ['ba', 'be', 'be-tarask', 'bg', 'bxr', 'cv', 'kk', 'ky', 'mk', 'mn', 'ru', 'rue', 'sah', 'sr', 'tg', 'tt', 'tyv', 'uk'], # langues très bien dotées
...
# Définir les langues manquantes à collecter
MISSING_LANGUAGES = ["ab", "kbd", "koi"]
...
TARGET_TOKENS_BY_GROUP = {
    'A': 100000,
    'B': 90000,
    'C': 80000,    # objectif plus réaliste pour ces langues
    'D': 75000
}
...
# Déplacer process_text au niveau global pour qu'elle soit accessible partout
...
def select_valid_articles(candidates, num_needed, already_collected_ids, min_length, language_code,
    ...
    for member in candidates_to_check:
        ...
        # Récupérer le contenu pour vérifier la longueur
        content_params = {
            "action": "query",
            "format": "json",
            "prop": "extracts",
        ...
        try:
            ...
            logging.error(f"  Erreur lors de l'extraction du contenu pour {title}", exc_info=True)
...
def get_articles_by_category(language_code, category_name, category_target, num_articles=20, 
    ...
    Args:
        ...
        num_articles: Nombre maximal d'articles à récupérer
    ...
    try:
        # 1. Pour les articles ordonnés, toujours utiliser l'ordre standard
        ordered_params = {
            "action": "query",
            "format": "json",
            "list": "categorymembers",
            "cmtitle": full_category,
            "cmlimit": "250",   # assez pour trouver num_fixed_selection articles valides
            "cmtype": "page",
            "cmsort": "sortkey",
            "cmdir": "asc"      # ordre alphabétique croissant (A→Z)
        }

        ordered_response = requests.get(api_url, params=ordered_params).json()
        ordered_members = []
        if 'query' in ordered_response and 'categorymembers' in ordered_response['query']:
            ordered_members = ordered_response['query']['categorymembers']

        # 2. Pour les articles aléatoires, utiliser des stratégies diverses
        random_strategies = [
            {"sort": "sortkey", "dir": "desc"},  # Fin de l'alphabet
            {"sort": "timestamp", "dir": "asc"}, # Articles les plus anciens 
            {"sort": "timestamp", "dir": "desc"} # Articles les plus récents
        ]
        ...
        random_params = {
            ...
            "cmlimit": "250",
            "cmtype": "page",
            "cmsort": random_strategy["sort"],
            "cmdir": random_strategy["dir"]
        }

        random_response = requests.get(api_url, params=random_params).json()
        random_members = []
        if 'query' in random_response and 'categorymembers' in random_response['query']:
            random_members = random_response['query']['categorymembers']
        ...
        category_tokens = 0  # Total des tokens pour cette catégorie

        # Collecter et traiter les articles ordonnés
        fixed_articles = select_valid_articles(
            ordered_members,
            num_fixed_selection,  # On garde ce paramètre comme limite supérieure
        ...
        random_articles = select_valid_articles(
            ...
            num_random_selection,  # Limite supérieure
            already_collected_ids,
            params['min_char_length'],
            language_code,
            category_name,
...
def get_articles_from_subcategories(language_code, category_name, num_articles=10, max_depth=3, 
    ...
    if cached_subcategories is None:
        ...
        while subcategory_queue and len(all_subcategories) < 800:
            ...
            if depth <= max_depth:
                # Paramètres pour obtenir les sous-catégories
                params = {
                    "action": "query",
                    "format": "json",
                    "list": "categorymembers",
                ...
                try:
                    ...
                    if 'query' in response and 'categorymembers' in response['query']:
                        subcats = response['query']['categorymembers']
                        
                        # Ajouter les sous-catégories à la file d'attente
                        for subcat in subcats:
                            if 'title' in subcat:
                                subcategory_queue.append((subcat['title'], depth + 1))
                except Exception as e:
                    logging.error(f"Erreur lors de l'exploration des sous-catégories de {current_category}: {e}", exc_info=True)
    ...
    if attempt_number > 1 and len(all_subcategories) > 10:
        ...
        while new_queue and len(new_explored) < 300:  # limite le nb de nouvelles sous-catégories
            ...
            try:
                # Pause plus courte pour cette exploration secondaire
                time.sleep(random.uniform(*sleep_time))
                ...
                if 'query' in response and 'categorymembers' in response['query']:
                    new_subcats = response['query']['categorymembers']
                ...
                logging.error(f"Erreur lors de l'exploration secondaire de {current_cat}: {e}", exc_info=True)
    ...
    for subcat in selected_subcats:
        ...
        # Paramètres pour obtenir les articles de la sous-catégorie
        params = {
            "action": "query",
            "format": "json",
            "list": "categorymembers",
        ...
        try:
            ...
            if 'query' in response and 'categorymembers' in response['query']:
                ...
                if members:
                    ...
                    for member in selected_members:
                        ...
                        # Paramètres pour récupérer le contenu
                        content_params = {
                            "action": "query",
                            "format": "json",
                            "prop": "extracts",
            ...
            logging.error(f"Erreur lors de la récupération des articles de {subcat}: {e}", exc_info=True)
...
# Fonction pour récupérer des articles aléatoirement (hors catégorie)
def get_random_articles(language_code, num_articles=20, sleep_time=(1, 3),
    ...
    while articles_collected < num_articles and attempts < max_attempts:
        ...
        random_response = requests.get(api_url, params=random_params).json()
        page_id = random_response["query"]["random"][0]["id"]
        title = random_response["query"]["random"][0]["title"]
        ...
        content_params = {
            ...
            "format": "json",
            "prop": "extracts",
            "explaintext": "1",  # format texte (pas HTML)
            "pageids": page_id
        }
        ...
        try:
            ...
            print(f"Impossible d'extraire le contenu pour: {title} ({language_code})")
            
        # Pause pour éviter de surcharger les serveurs de Wikipédia
        time.sleep(random.uniform(*sleep_time))
...
def main():
    """
    Fonction principale pour exécuter la collecte ciblée des langues manquantes
    """
    logging.info("Démarrage de la collecte ciblée pour les langues manquantes")

    # Configuration des chemins de fichiers modifiés pour éviter d'écraser les données
    output_folder = "data/raw/temp_collection_final"
    os.makedirs(output_folder, exist_ok=True)
    
    resume_file = "resume_missing_final_state.json"  # Nouveau fichier d'état
    global_stats_dir = "results/metrics/collection/global/temp_final"
    os.makedirs(global_stats_dir, exist_ok=True)
    global_stats_path = f"{global_stats_dir}/global_stats_final.csv"
    ...
    if os.path.exists(resume_file):
        try:
            with open(resume_file, 'r', encoding='utf-8') as f:
                ...
                print(f"Reprise de la collecte ciblée. {len(already_processed_languages)} langues déjà traitées.")
    ...
    # Utiliser uniquement les langues ciblées
    target_languages = MISSING_LANGUAGES
    max_languages = len(target_languages)
    
    print(f"====== DÉMARRAGE DE LA COLLECTE CIBLÉE POUR {max_languages} LANGUES ======")
    print(f"Langues ciblées: {', '.join(target_languages)}")
...
if __name__ == "__main__":
    try:
        # Étape 1: Collecter les données manquantes
        global_stats, all_articles = main()
        
        # Étape 2: Fusionner avec les données existantes
        merge_with_existing_data()
        
        print("Collecte et fusion terminées avec succès!")
    except Exception as e:
