...
USAGE: `PYTHONPATH=src python src/corpus/scripts/create_corpus.py`
"""
...
from corpus.modules.config import (
    TIME_LIMIT,
    LANGUAGES,
    ALL_CATEGORIES,
    MAX_DEPTHS_BY_GROUP,
...
from corpus.modules.article_collector import ArticleCollector
from corpus.modules.data_manager import save_articles_to_csv
from corpus.modules.stat_manager import (
    CollectionStats,
...
def collect_articles(language_code: str, categories: List[str]) -> Tuple:
    ...
    try:
        ...
        available_categories = get_available_categories_for_language(language_code)

        print(
            f"\nCatégories disponibles ({len(available_categories)}): {', '.join(available_categories)}"
        )
        ...
        stats = CollectionStats(
            language_code, token_target, available_categories
        )
        ...
        for category in available_categories:
            ...
            if time.time() - start_time > effective_timeout:
                print(
                    f"⏱️ Limite de temps atteinte pour {language_code}. "
            ...
            print("\n" + "=" * 60)
            print(f"📂 Début collecte pour la catégorie: {category} ({translated_category})")
            print("=" * 60)
            ...
            while (
                ...
                try:
                    ...
                    if not category_articles:
                        print(
                            f"  Aucun article disponible pour {category} ({translated_category})"
                        )
                        break
                    ...
                    for article in category_articles:
                        ...
                        print(
                            f"  ➤ Article ajouté ({category}): {article['title']} "
                    ...
                    logging.error(
                        f"Erreur lors de la collecte pour {category}: {str(e)}",
                        exc_info=True,
                    )
                    continue
        ...
        for category in available_categories:
            if time.time() - start_time > effective_timeout:
                print(
                    "⏱️ Limite de temps atteinte pendant la collecte des sous-catégories."
                )
                break

            translated_category = CATEGORY_TRANSLATIONS[category][language_code]
            print("\n" + "-" * 60)
            print(f"📂 Début collecte sous-catégories pour: {category} ({translated_category})")
            print("-" * 60)
            ...
            while (
                ...
                try:
                    ...
                    for article in subcategory_articles:
                        ...
                        print(
                            f"  ➤ Article sous-catégorie ajouté ({category}): {article['title']} "
                            f"({article['token_count']} tokens)"
                        )

                    category_tokens += sub_tokens
                    print(
                        f"  ✅ Progression: {category_tokens}/{category_target} tokens collectés"
                    )

                except Exception as e:
                    logging.error(
                        f"Erreur lors de la collecte des sous-catégories pour {category}: {str(e)}",
                        exc_info=True,
                    )
                    break

        # Étape 3: Articles aléatoires
        tokens_collected = 0
        while tokens_collected < stats.random_token_target:
            batch_size = max(1, min(15, (stats.random_token_target - tokens_collected) // 200))
            try:
                rand_articles, rand_tokens = collector.collect_random(
                    num_articles=batch_size, sleep_time=(0.5, 2)
                )
                if not rand_articles:
                    break
                articles.extend(rand_articles)
                for article in random_articles:
                    stats.update_random_stats(article)
                    print(
                        f"  ➤ Article aléatoire ajouté: {article['title']} "
                        f"({article['token_count']} tokens)"
                    )
                tokens_collected += rand_tokens
            except Exception as e:
                logging.error(f"Erreur articles aléatoires: {e}", exc_info=True)
        ...
        return (
            ...
            (stats.limited_articles / stats.articles_count * 100 if stats.articles_count > 0 else 0),
        )

    except Exception as e:
        logging.error(f"Erreur critique lors de la collecte pour {language_code}: {str(e)}", exc_info=True)
        raise RuntimeError(f"Échec de la collecte pour {language_code}: {e}") from e
